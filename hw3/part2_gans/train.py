...
class GAN(pl.LightningModule):
    def __init__(
        ...
        # Let's add persistent=False, because without it val_noise will be saved with model parameters =>
        # it will be retrieved during model.load_state_dict.
        ...
        if self.truncation_trick:
            ...
            # pass
            truncated_noise_flatten = stats.truncnorm.rvs(-0.5, 0.5, size=(len(self.train_dataset) * self.noise_channels))
            truncated_noise = truncated_noise_flatten.reshape(len(self.train_dataset), self.noise_channels)
            self.val_noise.data = torch.tensor(truncated_noise).float()
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        input_noise = torch.randn(imgs.shape[0], self.noise_channels, device=device) # batch_size = imgs.shape[0]
        
        if optimizer_idx == 0:
            # Generator loss
            fake_imgs = self.gen(input_noise, labels)
