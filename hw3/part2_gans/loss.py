...
class GANLoss(nn.Module):
    ...
    def __init__(self, loss_type):
        ...
        self.BCE_loss1 = nn.BCEWithLogitsLoss().to(self.device)
        self.BCE_loss2 = nn.BCEWithLogitsLoss().to(self.device)
        self.BCE_loss3 = nn.BCEWithLogitsLoss().to(self.device)
    ...
    def forward(self, fake_scores, real_scores=None):
        ...
        if real_scores is None:
            ...
            if self.loss_type == 'non_saturating':
                y_real = torch.ones(fake_scores.shape[0], device=self.device)
                loss = self.BCE_loss1(fake_scores, y_real)
            ...
            # pass
            if self.loss_type == 'non_saturating':
                y_real = torch.ones(real_scores.shape[0], device=self.device)
                y_fake = torch.zeros(fake_scores.shape[0], device=self.device)
                loss = self.BCE_loss2(real_scores, y_real) + self.BCE_loss3(fake_scores, y_fake)

            elif self.loss_type == 'hinge':
                loss = torch.mean(F.relu(1.0 - real_scores), dim=0) + \
                       torch.mean(F.relu(1.0 + fake_scores), dim=0)
...
class ValLoss(nn.Module):
    ...
    def calc_fid(real_features, fake_features):
        ...
        sqrt_of_product = linalg.sqrtm(np.dot(cov_real, cov_fake))
        if np.iscomplexobj(sqrt_of_product):
            sqrt_of_product = sqrt_of_product.real

        FID_score = np.sum((mean_real - mean_fake) ** 2.0) + np.trace(cov_real + cov_fake - 2.0 * sqrt_of_product)

        return FID_score # TODO (2 points)

    @staticmethod
    def calc_is(fake_probs):
        eps = 1e-10
        p_y = np.expand_dims(fake_probs.mean(axis=0), 0) + eps
        kl_divergence = fake_probs * np.log((fake_probs / p_y) + eps)
        IS_score = np.exp(np.mean(np.sum(kl_divergence, axis=1)))

        return IS_score # TODO (2 points)
